...
const QUERY_SQL = 'SELECT tenant, apiKey FROM ?? WHERE userId = ?';

const handler = async (event, context) => {
  const { requestContext: { authorizer: { claims: { sub } } } } = event;
  try {
    const { rows } = await trackExecTime('MySQL Batch Query Latency', () => queryAsync(QUERY_SQL, [process.env.NWCKEYS_TABLE, sub]));
