...
const INSERT_SQL = 'SELECT SUM(instanceCount) count FROM ?? WHERE userId = ?';

const handler = async (event, context) => {
  const { requestContext: { authorizer: { claims: { sub } } } } = event;
  try {
    const { rows } = await trackExecTime('MySQL Batch Insert Latency', () => queryAsync(INSERT_SQL, [process.env.INSTANCES_TABLE, sub]));
